...
const ROLES = [
  { value: "admin",    label: "Admin",     desc: "Full access + user management",     color: "#ff3b30" },
  { value: "manager",  label: "Manager",   desc: "All data, cannot manage users",     color: "#ff9500" },
  { value: "staff",    label: "Staff",     desc: "CRM + Travel Desk operations",      color: "#0071e3" },
  { value: "readonly", label: "Read Only", desc: "View only, no data changes",        color: "#8e8e93" },
];
...
function RoleBadge({ role }: { role: string | null }) {
  const r = role ?? "staff";
  const color = roleColor[r] ?? "#8e8e93";
  return (
    <span style={{
...
export default function SettingsPage() {
  ...
  const [openSection, setOpenSection] = useState<"users" | "gas" | "sheets" | "db">("users");
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      else if (res.status === 403) toast.error("Admin access required to view users");
  ...
  const addUser = async () => {
    ...
    if (!newUser.password.trim()) return toast.error("Password is required");
    setAdding(true);
    try {
      const res = await fetch("/api/admin/users", {
  ...
  const Section = ({ id, title, icon, color, children }: {
    id: typeof openSection; title: string; icon: React.ReactNode; color: string; children: React.ReactNode;
  }) => (
    <div className="glass-card" style={{ marginBottom: "1rem", overflow: "hidden" }}>
      <button onClick={() => setOpenSection(openSection === id ? "users" : id)} style={{
        width: "100%", padding: "1.125rem 1.5rem", display: "flex", alignItems: "center",
        gap: "0.75rem", background: "none", border: "none", cursor: "pointer",
        borderBottom: openSection === id ? "1px solid var(--color-border)" : "none",
      }}>
        ...
        <span style={{ fontWeight: 600, fontSize: "0.9375rem", flex: 1, textAlign: "left", color: "var(--color-text-primary)" }}>{title}</span>
        <ChevronDown size={16} color="var(--color-text-tertiary)" style={{ transform: openSection === id ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} />
      </button>
      {openSection === id && <div style={{ padding: "1.5rem" }}>{children}</div>}
    </div>
  );
  ...
  return (
    <div style={{ padding: "1.5rem 2rem", maxWidth: 860, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "1.75rem" }}>
        <h1 style={{ fontSize: "1.625rem", fontWeight: 700, color: "var(--color-text-primary)" }}>Settings</h1>
      ...
      <Section id="users" title="Staff Accounts & Access Control"
        color="linear-gradient(135deg,#0071e3,#5856d6)" icon={<Shield size={18} color="white" />}>
        ...
        {/* Add user form */}
        {showAdd && (
          <div style={{ background: "rgba(0,113,227,0.05)", border: "1px solid rgba(0,113,227,0.2)", borderRadius: 12, padding: "1.25rem", marginBottom: "1.25rem" }}>
            <h3 style={{ fontWeight: 600, fontSize: "0.9rem", marginBottom: "1rem", color: "var(--color-text-primary)" }}>New Staff Account</h3>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "0.75rem", marginBottom: "1rem" }}>
              <Input label="Username / Email *" value={newUser.email} onChange={v => setNewUser(u => ({ ...u, email: v }))} placeholder="alice or alice@company.com" />
              <div>
                <label className="label">Password *</label>
                <div style={{ position: "relative" }}>
                  <input type={showPass ? "text" : "password"} className="input" value={newUser.password}
                    onChange={e => setNewUser(u => ({ ...u, password: e.target.value }))}
                    placeholder="Min 8 characters" style={{ paddingRight: "2.5rem" }} />
                  <button type="button" onClick={() => setShowPass(!showPass)} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--color-text-tertiary)" }}>
                    {showPass ? <EyeOff size={14} /> : <Eye size={14} />}
                  </button>
                </div>
              </div>
              <Input label="Display Name" value={newUser.name} onChange={v => setNewUser(u => ({ ...u, name: v }))} placeholder="Alice Smith" />
              <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button className="btn-secondary" onClick={() => setShowAdd(false)}><X size={13} /> Cancel</button>
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: "1rem" }}>
          {ROLES.map(r => (
            <div key={r.value} style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 10px", borderRadius: 8, background: "rgba(120,120,128,0.08)", fontSize: "0.75rem" }}>
        ...
        <div style={{ border: "1px solid var(--color-border)", borderRadius: 12, overflow: "hidden" }}>
          {loadingUsers ? (
            <div style={{ padding: "2rem", textAlign: "center", color: "var(--color-text-tertiary)", fontSize: "0.875rem" }}>Loading users…</div>
          ) : users.length === 0 ? (
            <div style={{ padding: "2rem", textAlign: "center", color: "var(--color-text-tertiary)", fontSize: "0.875rem" }}>
              No staff accounts yet.<br />Create the first account above.
            </div>
          ) : (
                  ...
                  <tr key={u.id}>
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: `linear-gradient(135deg, ${roleColor[u.role ?? "staff"] ?? "#8e8e93"}, ${roleColor[u.role ?? "staff"] ?? "#8e8e93"}88)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.8125rem", fontWeight: 700, color: "white", flexShrink: 0 }}>
                          {(u.name ?? u.email)[0].toUpperCase()}
                        </div>
                        <span style={{ fontWeight: 500, color: "var(--color-text-primary)" }}>{u.name ?? "—"}</span>
                      </div>
                    </td>
                    <td><code style={{ fontSize: "0.8125rem", color: "var(--color-text-secondary)" }}>{u.email}</code></td>
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 4 }}>
                        ...
                        <button title="Delete" style={{ padding: "0.25rem 0.5rem", background: "rgba(255,59,48,0.1)", border: "1px solid rgba(255,59,48,0.2)", borderRadius: 8, cursor: "pointer", color: "#ff3b30", display: "flex", alignItems: "center" }}
        ...
        {editUser && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
            <div className="glass-card-elevated" style={{ width: "100%", maxWidth: 440, padding: "1.5rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                <Input label="Display Name" value={editUser.name} onChange={v => setEditUser(e => e ? { ...e, name: v } : null)} placeholder="Alice Smith" />
                <div>
                  <label className="label">Role</label>
                ...
                <div>
                  <label className="label">New Password <span style={{ color: "var(--color-text-tertiary)", fontWeight: 400 }}>(leave blank to keep current)</span></label>
      ...
      {/* ── SECTION 2: Google Apps Script ────────────────────────────────────── */}
      <Section id="gas" title="Google Apps Script (Drive + Sheets)"
        color="linear-gradient(135deg,#4285f4,#34a853)" icon={<ExternalLink size={18} color="white" />}>

        <div style={{ marginBottom: "1rem" }}>
          <Input label="GAS Web App URL"
            value={settings.gas_web_app_url}
            onChange={v => setSettings(s => ({ ...s, gas_web_app_url: v }))}
            placeholder="https://script.google.com/macros/s/…/exec"
            hint="Deploy gas/Code.gs as a Web App → Anyone → copy the URL here" />
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: "1rem" }}>
          ...
          {gasStatus !== "idle" && (
            <span style={{ fontSize: "0.8125rem", display: "flex", alignItems: "center", gap: 4, color: gasStatus === "ok" ? "var(--color-success)" : "var(--color-danger)", fontWeight: 500 }}>
        ...
        <div style={{ background: "rgba(120,120,128,0.07)", borderRadius: 12, padding: "1rem", fontSize: "0.8125rem" }}>
          <p style={{ fontWeight: 600, marginBottom: "0.5rem" }}>Setup instructions:</p>
          <ol style={{ paddingLeft: "1.25rem", lineHeight: 2, color: "var(--color-text-secondary)" }}>
            ...
            <li>Paste contents of <code>gas/Code.gs</code> from your project</li>
            <li>Set your Google Drive Folder ID in the <code>CONFIG</code> block</li>
            <li>Deploy → New Deployment → Web App → Execute as <strong>Me</strong> → Access: <strong>Anyone</strong></li>
            <li>Copy the Web App URL and paste it above, then click Save</li>
          </ol>
        </div>
      </Section>

      {/* ── SECTION 3: Google Sheets ─────────────────────────────────────────── */}
      <Section id="sheets" title="Google Sheets & Drive Folder"
        color="linear-gradient(135deg,#34a853,#0f9d58)" icon={<Save size={18} color="white" />}>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.875rem" }}>
          <Input label="Spreadsheet ID" value={settings.registration_sheet_id}
            onChange={v => setSettings(s => ({ ...s, registration_sheet_id: v }))}
            placeholder="1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgVE2upms"
            hint="From Sheet URL: /spreadsheets/d/[ID]/edit" />
          <Input label="Drive Folder ID" value={settings.drive_folder_id}
            onChange={v => setSettings(s => ({ ...s, drive_folder_id: v }))}
            placeholder="1A2B3C4D5E6F7G8H9I…"
            hint="From Drive folder URL: /folders/[ID]" />
          <Input label="Registration Sheet Tab Name" value={settings.registration_sheet_name}
            onChange={v => setSettings(s => ({ ...s, registration_sheet_name: v }))}
            placeholder="Form Responses 1" />
          <Input label="Travel Desk Sheet Tab Name" value={settings.travel_sheet_name}
      ...
      {/* ── SECTION 4: Database ───────────────────────────────────────────────── */}
      <Section id="db" title="Database Setup (Neon PostgreSQL)"
        color="linear-gradient(135deg,#1a1a2e,#16213e)" icon={<CheckCircle2 size={18} color="white" />}>
        <p style={{ fontSize: "0.875rem", color: "var(--color-text-secondary)", marginBottom: "1rem" }}>
          Run these commands once after connecting your Neon database:
        </p>
        {[
          { label: "Option A — Neon SQL Editor:", cmd: "Paste neon-schema.sql into console.neon.tech → SQL Editor → Run" },
          { label: "Option B — Via API (server running):", cmd: "GET /api/setup?key=delegate-admin-seed-2024" },
        ].map(({ label, cmd }) => (
          <div key={label} style={{ marginBottom: "0.75rem", background: "rgba(120,120,128,0.07)", borderRadius: 10, padding: "0.875rem" }}>
            <p style={{ fontSize: "0.75rem", color: "var(--color-text-secondary)", marginBottom: 4 }}>{label}</p>
            <code style={{ fontSize: "0.8125rem", color: "var(--color-accent)", wordBreak: "break-all" }}>{cmd}</code>
          </div>
        ))}
      </Section>
